class AddAclManagerTo<%= table_name.camelize %> < ActiveRecord::Migration
  def change
    ...
    create_table :acl_manager_roles_<%= table_name %> do |t|
    ...
    add_foreign_key :acl_manager_roles_<%= table_name %>, :<%= table_name %>
    add_foreign_key :acl_manager_roles_<%= table_name %>, :acl_manager_roles
  end
end
